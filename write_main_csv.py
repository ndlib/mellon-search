...
def write_main_csv(csv_directory, json_data):
    ...
    with open(csv_directory + '/' + csv_filename, 'w') as main_csv:
        fieldnames = ['Label', 'Description', 'License', 'Attribution', 'Sequence_filename', \
...
def _create_first_row(json_data):
    ...
    row['License'] = '<a href="http://rightsstatements.org/vocab/NoC-US/1.0/" target="_blank">No Copyright - United States</a>'
...
def _create_metadata_rows(json_data):
    ...
    rows = {}
    rows = []
    rows.append(_create_metadata_row(json_data, 'recordId', 'Accession Number'))
    ...
    #print(rows)
    rows.append(_create_metadata_row(json_data, 'creationDate', 'Creation Date'))
    rows.append(_create_metadata_row(json_data, 'displayDate', 'Display Date'))
    rows.append(_create_metadata_row(json_data, 'title', 'Title'))
    rows.append(_create_metadata_row(json_data, 'classification', 'Classification'))
    rows.append(_create_metadata_row(json_data, 'media', 'Media'))
    rows.append(_create_metadata_row(json_data, 'displayDimensions', 'Dimensions'))
    rows.append(_create_exhibition_metadata_row(json_data, 'exhibition', 'Exhibition'))
    rows.append(_create_metadata_array_row(json_data, 'relatedObjects', 'Related Objects'))
    rows.append(_create_metadata_array_row(json_data, 'bibliography', 'Bibliography'))
...
def _create_metadata_array_row(json_data, node_name, metadata_label):
    ...
    if node_name in json_data:
        ...
        if metadata_value > "":
            row = _create_metadata_row_from_string(metadata_label, metadata_value)
    return row
